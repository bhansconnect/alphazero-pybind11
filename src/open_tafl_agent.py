...
def eval_position(gs, mcts, agent, args, time_limit):
    ...
    pi = pi.cpu().numpy()
    high_prob = {
        move_to_string(x[0], height, width): pi[x[0]] for x in np.argwhere(pi > 0.05)
    }
    ...
    probs = mcts.probs(
        calc_temp(
            args.start_temp, args.end_temp, args.temp_half_life, gs.current_turn()
        )
    )
    high_prob = {
        move_to_string(x[0], height, width): probs[x[0]]
    ...
    print(f"status MCTS best: {move_to_string(np.argmax(probs), height, width)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fpu-redux", type=float, default=0.25, help="First play urgency reduction"
    )
    parser.add_argument(
        "--game",
        type=str,
    ...
    if args.network == "latest":
        nn_files = [
            os.path.basename(x)
            for x in sorted(glob.glob(os.path.join(args.folder, f"*-{name}-*.pt")))
        ]
        args.network = nn_files[-1]
    start = time.time()
    print(f"status Loading network {args.network} in {os.path.abspath(args.folder)}")
    try:
        nn = neural_net.NNWrapper.load_checkpoint(Game, args.folder, args.network)
