...
[[nodiscard]] std::string OpenTaflGS::to_bytes() const {
  std::string out;
...
[[nodiscard]] OpenTaflGS OpenTaflGS::from_bytes(const std::string& data) {
  if (data.size() < kFixedHeaderSize + 4) {
    throw std::runtime_error("OpenTaflGS::from_bytes: data too short");
  }
  BoardTensor board{};
  size_t off = 0;
  const size_t board_bytes =
  ...
  if (off + num_entries * (board_bytes + 2) != data.size()) {
    throw std::runtime_error(
        "OpenTaflGS::from_bytes: repetition entry count mismatch");
  }
  ...
  return OpenTaflGS(board, player, turn, max_turns, rep,
                    std::move(counts), intern);
}

[[nodiscard]] std::unique_ptr<GameState> OpenTaflGS::copy() const noexcept {
  return std::make_unique<OpenTaflGS>(board_, player_, turn_, max_turns_,
